// https://learn.microsoft.com/en-us/windows/win32/winsock/complete-client-code
#define WIN32_LEAN_AND_MEAN
#include <windows.h>
#include <winsock2.h>
#include <ws2tcpip.h>

// Need to link with Ws2_32.lib, Mswsock.lib, and Advapi32.lib
#pragma comment (lib, "Ws2_32.lib")
#pragma comment (lib, "Mswsock.lib")
#pragma comment (lib, "AdvApi32.lib")

import std;
import shared;

auto main(int argc, char** argv) -> int
try
{
    if (argc != 2)
    {
        std::println("usage: {} server-name", argv[0]);
		std::println("If server-name is not provided, it defaults to localhost.");
    }

    // Initialize Winsock
    auto ws = WsaContext{};

    // Resolve the server address and port
    auto addrInfo =
        [](const char* serverName) -> AddrInfoUniquePtr
        {
            auto hints = addrinfo{
                .ai_family = AF_UNSPEC,
                .ai_socktype = SOCK_STREAM,
                .ai_protocol = IPPROTO_TCP
            };
            auto result = AddrInfoUniquePtr{};
            constexpr auto defaultPort = "27915";
            if (auto addrResult = getaddrinfo(serverName, defaultPort, &hints, std::out_ptr(result)); addrResult != 0)
				throw WinSockError{ addrResult, "getaddrinfo failed" };
            return result;
		}(argc == 2 ? argv[1] : "localhost");

    // Attempt to connect to an address until one succeeds
    auto clientSocket = 
		[addrInfo = std::move(addrInfo)] -> Socket
        {
            for (addrinfo* ptr = addrInfo.get(); ptr != nullptr; ptr = ptr->ai_next)
            {
                // Create a SOCKET for connecting to server
                auto clientSocket = Socket{ socket(ptr->ai_family, ptr->ai_socktype, ptr->ai_protocol) };
                if (not clientSocket.IsValid())
                    throw WinSockError{ WSAGetLastError(), "socket failed" };
                // Connect to server.
                if (connect(clientSocket.handle, ptr->ai_addr, (int)ptr->ai_addrlen) != SOCKET_ERROR)
                    return clientSocket;
            }
            throw std::runtime_error{ "Unable to connect to server!" };
        }();

    // Send an initial buffer
    [clientSocket = std::move(clientSocket)]
    {
        // We expect this to echo back from the server.
        auto bytesSent = clientSocket.Send("Hello, world!");
        std::println("Bytes Sent: {}", bytesSent);

        // Receive until the peer closes the connection
        while (true)
        {
            if (auto received = clientSocket.Recv(); received)
            {
                std::println("Bytes received: {}: {}", received->size(), *received);
            }
            else
            {
                std::println("Connection closed by peer.");
				break;
            }
        };
    }();

    return 0;
}
catch (const std::exception& e)
{
    std::println("Error: {}", e.what());
    return 1;
}
